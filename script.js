...
function handleBackspace() {
    ...
    renderState.newIndex = typingEngineState.index;
    renderState.onlyCharChange = true;
    renderState.deltas.push({type: "backspace", index: typingEngineState.index});
...
function renderClassic() {
    if(renderState.onlyCharChange && !renderState.layoutDirty && typingEngineState.visibleCharMap.length) {
        renderDeltaBatch();
    } else {
        renderClassicMode();
    }
...
function startTimer() {
    ...
    timerID = setInterval(() => {
        ...
        typingEngineState.timeLeft = Math.max(0, typingEngineState.duration - timePassed);
